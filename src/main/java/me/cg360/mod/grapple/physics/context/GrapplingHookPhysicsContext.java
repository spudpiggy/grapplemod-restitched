package me.cg360.mod.grapple.physics.context;
...
import me.cg360.mod.grapple.network.serialize.serverbound.GrappleEndMessage;
import me.cg360.mod.grapple.network.serialize.serverbound.PlayerMovementMessage;
...
public class GrapplingHookPhysicsContext {
	...
	public GrapplingHookPhysicsContext(int grapplehookEntityId, int entityId, Level world, int controllerId, CustomizationVolume custom) {
		...
		if (this.custom != null) {
			this.playerMovementMult = this.custom.get(GrappleModCustomizationProperties.MOVE_SPEED_MULTIPLIER.get());
			this.maxLen = custom.get(GrappleModCustomizationProperties.MAX_ROPE_LENGTH.get());
		}
		...
		if (custom != null && custom.get(GrappleModCustomizationProperties.ROCKET_ATTACHED.get())) {
			GrappleModClient.get().updateRocketRegen(custom.get(GrappleModCustomizationProperties.ROCKET_FUEL_DEPLETION_RATIO.get()), custom.get(GrappleModCustomizationProperties.ROCKET_REFUEL_RATIO.get()));
		}
	}
	...
	public void updatePlayerPos() {
		...
		// is motor active?
		boolean motor = false;
		if (this.custom.get(GrappleModCustomizationProperties.MOTOR_ATTACHED.get())) {
			boolean isActive = this.custom.get(GrappleModCustomizationProperties.MOTOR_ACTIVATION.get()).isActive(GrappleModKey.key_motoronoff);
		...
		for (GrapplinghookEntity hookEntity : this.grapplehookEntities) {
			...
			if (this.custom.get(GrappleModCustomizationProperties.BLOCK_PHASE_ROPE.get())) {
			...
			double remainingLength = motor
					? Math.max(this.custom.get(GrappleModCustomizationProperties.MAX_ROPE_LENGTH.get()), hookEntity.ropeLength) - distToAnchor
		...
		if (motor) {
			...
			if (this.custom.get(GrappleModCustomizationProperties.DOUBLE_HOOK_ATTACHED.get()) && this.grapplehookEntities.size() == 1) {
				...
				if (isdouble && !this.custom.get(GrappleModCustomizationProperties.SINGLE_ROPE_PULL.get())) {
					dopull = false;
				}
			}
			...
			double accel = this.custom.get(GrappleModCustomizationProperties.MOTOR_ACCELERATION.get()) / this.grapplehookEntities.size();
			...
			for (GrapplinghookEntity hookEntity : this.grapplehookEntities) {
				...
				if (pull.dot(facing) > 0 || this.custom.get(GrappleModCustomizationProperties.MOTOR_WORKS_BACKWARDS.get())) {
					if (this.custom.get(GrappleModCustomizationProperties.SMART_MOTOR.get()) && this.grapplehookEntities.size() > 1) {
			...
			if (this.custom.get(GrappleModCustomizationProperties.DOUBLE_SMART_MOTOR.get()) && this.grapplehookEntities.size() > 1) {
				...
				for (GrapplinghookEntity hookEntity : this.grapplehookEntities) {
					...
					if (pull.dot(facing) > 0 || this.custom.get(GrappleModCustomizationProperties.MOTOR_WORKS_BACKWARDS.get())) {
						...
						if (hookEntity.isInDoublePair) {
							if (!this.custom.get(GrappleModCustomizationProperties.SINGLE_ROPE_PULL.get())) {
								dopull = false;
							}
						}
					}
			...
			double pullmult = 1;
			if (this.custom.get(GrappleModCustomizationProperties.SMART_MOTOR.get()) && totalPull.y > 0 && !(this.onGroundTimer > 0 || entity.onGround())) {
				...
				if (pulll > this.custom.get(GrappleModCustomizationProperties.MOTOR_ACCELERATION.get())) {
					pulll = this.custom.get(GrappleModCustomizationProperties.MOTOR_ACCELERATION.get());
				}
			...
			if (this.motion.dot(totalPull) > 0) {
				if (this.motion.proj(totalPull).length() + totalPull.scale(pullmult).length() > this.custom.get(GrappleModCustomizationProperties.MAX_MOTOR_SPEED.get())) {
					pullmult = Math.max(0, (this.custom.get(GrappleModCustomizationProperties.MAX_MOTOR_SPEED.get()) - this.motion.proj(totalPull).length()) / totalPull.length());
				}
			}

			// sideways dampener
			if (this.custom.get(GrappleModCustomizationProperties.MOTOR_DAMPENER.get()) && totalPull.length() != 0) {
		...
		// forcefield
		if (this.custom.get(GrappleModCustomizationProperties.FORCEFIELD_ATTACHED.get())) {
			Vec blockPush = checkRepel(playerPos, entity.level());
			blockPush.mutableScale(this.custom.get(GrappleModCustomizationProperties.FORCEFIELD_FORCE.get()) * 0.5f)
					 .mutableMultiply(0.5D, 2.0D, 0.5D);
			this.motion.mutableAdd(blockPush);
		}

		// rocket
		if (this.custom.get(GrappleModCustomizationProperties.ROCKET_ATTACHED.get())) {
	...
	public Vec rocket(Entity entity) {
		...
		double rocket_force = this.custom.get(GrappleModCustomizationProperties.ROCKET_FORCE.get()) * 0.225 * this.rocketProgression;
		double yaw = entity.getYRot();
		double pitch = this.custom.get(GrappleModCustomizationProperties.ROCKET_ANGLE.get()) - entity.getXRot();
