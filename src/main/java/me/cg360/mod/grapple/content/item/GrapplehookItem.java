package me.cg360.mod.grapple.content.item;

import com.yyon.grapplinghook.client.GrappleModClient;
import me.cg360.grapplemod.client.keybind.KeyBindingManagement;
...
import me.cg360.mod.grapple.content.item.type.DroppableItem;
import me.cg360.mod.grapple.content.item.type.KeypressItem;
import me.cg360.mod.grapple.customization.type.AttachmentProperty;
import me.cg360.mod.grapple.customization.type.CustomizationProperty;
import me.cg360.mod.grapple.network.NetworkManager;
import me.cg360.mod.grapple.network.serialize.clientbound.DetachSingleHookMessage;
import me.cg360.mod.grapple.network.serialize.clientbound.GrappleDetachMessage;
import me.cg360.mod.grapple.network.serialize.serverbound.KeypressMessage;
...
public class GrapplehookItem extends Item implements KeypressItem, DroppableItem {
	...
	public void onCustomKeyDown(ItemStack stack, Player player, KeypressItem.Keys key, boolean ismainhand) {
		if (player.level().isClientSide) {
			if (key == KeypressItem.Keys.LAUNCHER) {
				if (this.getCustomization(stack).get(GrappleModCustomizationProperties.ENDER_STAFF_ATTACHED.get()))
				...
				if (custom.get(GrappleModCustomizationProperties.ROCKET_ATTACHED.get()))
		...
		if (key == KeypressItem.Keys.THROWBOTH || (!custom.get(GrappleModCustomizationProperties.DOUBLE_HOOK_ATTACHED.get()) && isEitherSingleHandThrowKeyDown)) {
	...
	public void onCustomKeyUp(ItemStack stack, Player player, KeypressItem.Keys key, boolean ismainhand) {
		if (player.level().isClientSide) {
	    	...
	    	if (custom.get(GrappleModCustomizationProperties.DETACH_HOOK_ON_KEY_UP.get())) {
	...
	public void throwBoth(ItemStack stack, Level worldIn, LivingEntity entityLiving, boolean rightHand) {
	    ...
	    boolean shouldThrowLeft = !custom.get(GrappleModCustomizationProperties.DOUBLE_HOOK_ATTACHED.get()) || angle == 0;
	...
	public boolean throwRight(ItemStack stack, Level worldIn, LivingEntity entityLiving, boolean righthand) {
		...
		boolean isNotDouble = !custom.get(GrappleModCustomizationProperties.DOUBLE_HOOK_ATTACHED.get()) || angle == 0;
    ...
    public double getSingleHookAngle(LivingEntity entity, CustomizationVolume custom) {
		return entity.isCrouching()
				? custom.get(GrappleModCustomizationProperties.HOOK_THROW_ANGLE_ON_SNEAK.get())
				: custom.get(GrappleModCustomizationProperties.HOOK_THROW_ANGLE.get());
    }

	public double getDoubleHookAngle(LivingEntity entity, CustomizationVolume custom) {
		return entity.isCrouching()
				? custom.get(GrappleModCustomizationProperties.DOUBLE_HOOK_ANGLE_ON_SNEAK.get())
				: custom.get(GrappleModCustomizationProperties.DOUBLE_HOOK_ANGLE.get());
	}
	...
	public void appendHoverText(ItemStack stack, Level world, List<Component> list, TooltipFlag par4) {
		...
		if (Screen.hasShiftDown()) {
			...
			if (custom.get(GrappleModCustomizationProperties.ENDER_STAFF_ATTACHED.get())) {
			...
			if (custom.get(GrappleModCustomizationProperties.MOTOR_ATTACHED.get())) {

				Component text = switch (custom.get(GrappleModCustomizationProperties.MOTOR_ACTIVATION.get())) {
			...
			if (custom.get(GrappleModCustomizationProperties.DOUBLE_HOOK_ATTACHED.get())) {
				if (!custom.get(GrappleModCustomizationProperties.DETACH_HOOK_ON_KEY_UP.get())) {
			...
			if (custom.get(GrappleModCustomizationProperties.HOOK_REEL_IN_ON_SNEAK.get())) {
